...
package week7.src.main.scala.fpinscala.testing

import week7.src.main.scala.fpinscala.state.{RNG, State}
...
case class Gen[A] (sample :State[RNG,A]) {
  ...
  def listOfN(size: Gen[Int]): Gen[List[A]] = {
    this.flatMap(a => listOfN(size.toStream(42).head))
  }
  ...
  //Gen.unit (true)->1.0, Gen.unit (false)->2.0

  def union(that: Gen[A]): Gen[A] = {
    Gen.boolean.flatMap(value => if (value) this else that)
  }
